...
def _interface_messages(request, langcode):
    ...
    if username != None:
        output[
            'username'] = username  # leave 'username' key unset if no session
...
def select_language(request):  # /requests
    ...
    return _create_page(request, 'en', content,
                        'requestoid/select_language.html')
...
def auth(request):  # /requests/auth
    ...
    request.session['request_token_secret'] = request_token.secret.decode(
        'utf-8')
    request.session['return_to'] = request.META['HTTP_REFERER']
    return HttpResponseRedirect(
        redirect
    )  # This hands the user off to Wikimedia; user returns to the website via the callback view which implements the session
...
def callback(request):  # /requests/callback
    ...
    request.session['access_token_secret'] = access_token.secret.decode(
        'utf-8')
            ...
            if g['pagetitle'] != "":
                if 'pageid' in p:  # Ready to save to database; pageid is only passed as a parameter if workflow below has been completed
                    ...
                    R = transactions.new_entry(
                        p['pageid'], p['pagetitle'], userid, username,
                        p['request_language'] + 'wiki', p['summary'],
                        p['note'], categories, wikiprojects,
                        p['request_language'])

                    return HttpResponseRedirect("/requests/" + langcode +
                                                "/request/" + str(R.id))
                    ...
                    if pageid == 0:  # new article workflow
                        ...
                        content['category_textarea'] = wiki.GetCategories(
                            request_language, pageid)
                        content['wikiprojects_explanation'] = _(
                            'WikiProjects are retrieved automatically from Wikipedia. You may add additional projects if you wish.'
                        )
                        content[
                            'wikiprojects_textarea'] = wiki.GetWikiProjects(
                                request_language, pagetitle)

                    content['note_label'] = _('Add a note')
                    content['note_explanation'] = _(
                    ...
                    return _create_page(request, langcode, content,
                                        'requestoid/add_details.html')
...
def request(request, langcode, reqid):  # /requests/en/request/12345
    ...
    if 'categories' in p:
        ...
        if username != None and R.page_id == 0:
            ...
            for category in taken_out:
                query = models.Categories.objects.filter(
                    cat_title=category, request_id=reqid)
            ...
            for category in added_in:
                transactions.add_category(
                    R, R.wiki[:-4], category, username=username, userid=userid)
    ...
    if 'wikiprojects' in p:
        ...
        if username != None:
            ...
            new_wikiprojects = [
                x for x in p['wikiprojects'].split("\r\n")
                if x != '' and x != ' '
            ]
            ...
            for wikiproject in taken_out:
                query = models.WikiProjects.objects.filter(
                    project_title=wikiproject, request_id=reqid)
    ...
    if 'newnote' in p:
        if username != None and p['newnote'] != '' and p['newnote'] != ' ' and p['newnote'] != '\r\n':
            ...
            transactions.add_note(
                R, p['newnote'], username=username, userid=userid)
    ...
    status = ['Open', 'Complete',
              'Declined']  # 0 = open; 1 = complete; 2 = declined
    ...
    C = models.Categories.objects.filter(
        request_id=reqid).order_by('cat_title')
    W = models.WikiProjects.objects.filter(
        request_id=reqid).order_by('project_title')

    for note in N:
        try:
            ts = arrow.get(note.timestamp,
    ...
    content = {
        ...
        'save_label':
        _('Save'),
        'edit_categories_on_wikipedia_label':
        _('Categories must be changed directly on Wikipedia'),
        'requestdata':
        requestdata
    }

    if username == None:
        content['list_edit_explanation'] = _(
...
def search(request, langcode):  # /requests/en/search
    ...
    if 'searchterm' in g:
        ...
        if searchterm != '' and searchterm != ' ':
            R = transactions.retrieve_requests(searchterm, searchtype,
                                               g['language'])
    ...
    return _create_page(request, langcode, content,
                        'requestoid/list_start.html')


def help(request, langcode):  # /requests/en/help
    translation.use_language = langcode
    content = {'headline': _('Help'), 'intro': _('help_body')}
...
def about(request, langcode):  # /requests/en/about
    translation.use_language = langcode
    content = {
        'headline': _('About Wikipedia Requests'),
        'intro': _('about_body')
    }
    return _create_page(request, langcode, content, 'requestoid/help.html')
    ...
    if 'step' in p:  # We are on a step other than the initial landing page
        if p['step'] == 'mapping':
            ...
            if file.name.endswith('.csv') or file.name.endswith('.tsv'):
                ...
                content = {
                    'headline':
                    _('bulkimport-headline'),
                    'separator_label':
                    _('Separator'),
                    'separator_explanation':
                    _('bulkimport-separator-explanation'),
                    'which_field':
                    _('bulkimport-whichfieldisthis'),
                    'which_field_ignorecolumn':
                    _('bulkimport-whichfieldisthis-ignore'),
                    'which_field_language':
                    _('bulkimport-whichfieldisthis-langcode'),
                    'which_field_pagetitle':
                    _('bulkimport-whichfieldisthis-pagetitle'),
                    'which_field_summary':
                    _('Summary'),
                    'which_field_note':
                    _('bulkimport-whichfieldisthis-note'),
                    'which_field_categories':
                    _('Categories'),
                    'which_field_wikiprojects':
                    _('WikiProjects'),
                    'submit_button':
                    _('Go'),
                    'sample_data':
                ...
                return _create_page(request, langcode, content,
                                    'requestoid/bulk_mapping.html')
            else:
                content = {
                    'headline':
                    _('bulkimport-error-wrongtype'),
                    'explanation':
                    _('bulkimport-error-wrongtype-details').format(file.name)
                }
                return _create_page(request, langcode, content,
                                    'requestoid/error.html')
            ...
            for k, v in p.items():
                if k.startswith('which_field_'):
                    if v in matched_column_names:  # bad
                        content = {
                            'headline':
                            _('bulkimport-error-dupecolumns'),
                            'explanation':
                            _('bulkimport-error-dupecolumns-details').format(v)
                        }
                        return _create_page(request, langcode, content,
                                            'requestoid/error.html')
                    else:
                        ...
                        if v not in ['', '_ignore'
                                     ]:  # multiple ignored columns are allowed
                            matched_column_names.append(v)

            # Check if any columns are missing:
            for column in [
                    'language', 'pagetitle', 'summary', 'note', 'categories',
                    'wikiprojects'
            ]:
                if column not in matched_column_names:
                    content = {
                        'headline':
                        _('bulkimport-error-missingcolumn'),
                        'explanation':
                        _('bulkimport-error-missingcolumn-details').format(
                            column)
                    }
                    return _create_page(request, langcode, content,
                                        'requestoid/error.html')
            ...
            transactions.bulk_create.delay(manifest, filename, code, username,
                                           userid)

            content = {
                'headline': _('bulkimport-success'),
                'explanation': _('bulkimport-success-details').format(langcode)
            }
            return _create_page(request, langcode, content,
                                'requestoid/bulk_results.html')

    # No forms filled in yet.
    content = {
        'headline': _('bulkimport-headline'),
        'intro': _('bulkimport-intro'),
        'button': _('bulkimport-button')
    }
    return _create_page(request, langcode, content,
                        'requestoid/bulk_start.html')
