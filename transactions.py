...
def retrieve_requests(searchterm, searchtype, language):
    ...
    if searchtype == 'article':
        ...
        R = {
            'open':
            models.Requests.objects.filter(
                page_title=searchterm, wiki=database, status=0),
            'complete':
            models.Requests.objects.filter(
                page_title=searchterm, wiki=database, status=1)
        }
    elif searchtype == 'category':
        if searchterm[:9] == 'Category:':
            searchterm = searchterm[9:]
        C1 = models.Categories.objects.filter(
            cat_title=searchterm, wiki=database, request__status=0)
        R1 = [entry.request for entry in C1]
        C2 = models.Categories.objects.filter(
            cat_title=searchterm, wiki=database, request__status=1)
        ...
        W1 = models.WikiProjects.objects.filter(
            project_title=searchterm, wiki=database, request__status=0)
        R1 = [entry.request for entry in W1]
        W2 = models.WikiProjects.objects.filter(
            project_title=searchterm, wiki=database, request__status=1)
...
def create_request_entry(page_id,
                         page_title,
                         userid,
                         username,
                         wiki,
...
def add_wikiproject(request,
    ...
    W.save()
    _post_log(
        request, 'addwikiproject', W.id, username=username, userid=userid)
    return W


def new_entry(page_id,
              page_title,
              userid,
              username,
              wiki,
...
def bulk_create(manifest, filename, code, username, userid):
    ...
    _post_log(
        None, 'importspreadsheet', S.id, username=username, userid=userid)
    ...
    for entry in manifest:
        ...
        if pageid > 0:
            entry['categories'] = wiki.GetCategories(entry['language'],
                                                     pageid).split('\n')
